...
int inserisci_in_coda(int array[], int *n, int numero) // spostare l'incremento dopo l'inserimento
{
...
void modifica(int array[], int posizione, int valore) // controlla
{
	array[posizione] = valore;
}
...
void esegui_operazione(int s, int numeri[], int *n)
{
	if (s == 1)
	{
		stampa(numeri, *n); // passo il valore di n puntato
	}
	if (s == 2)
	{
		...
		scanf("%d", &numero);
		posizione = inserisci_in_coda(numeri, n, numero); // viene passato l'indirizzo di n e poi creo un puntatore nella funzione
	...
	if (s == 3)
	{
		...
		scanf("%d", &valore);
		posizione = ricerca(numeri, *n, valore); // assegno il valore della posizione fuori dalla condizione dell'if
		if (posizione > -1)
		{
